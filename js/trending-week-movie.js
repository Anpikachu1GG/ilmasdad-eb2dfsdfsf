document.addEventListener('DOMContentLoaded', async () => {
    ...
    const fetchTVShowDetails = async (tvShowId) => {
        const [vnData, enData, creditsData] = await Promise.all([
            fetchJSON(`https://api.themoviedb.org/3/movie/${tvShowId}?api_key=${tmdbApiKey}&language=vi-VN`),
            fetchJSON(`https://api.themoviedb.org/3/movie/${tvShowId}?api_key=${tmdbApiKey}&language=en-US`),
            fetchJSON(`https://api.themoviedb.org/3/movie/${tvShowId}/credits?api_key=${tmdbApiKey}&language=en-US`)
        ]);

        return vnData && enData ? {
            nameVN: vnData.title || enData.title,
            nameEN: enData.title || vnData.title,
    ...
    const fetchTrendingMovies = (page = 1) => 
        fetchJSON(`https://api.themoviedb.org/3/trending/movie/week?api_key=${tmdbApiKey}&language=vi-VN&page=${page}`);
